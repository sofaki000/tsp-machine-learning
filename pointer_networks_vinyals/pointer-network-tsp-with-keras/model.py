...
class PointerLSTM(keras.layers.Layer):
    ...
    def call(self, x, training=None, mask=None, last_decoder_states=None):
        """
        :param Tensor x: Should be the output of the decoder
        :param Tensor last_decoder_states: last state of the decoder
        ...
        if last_decoder_states:
            initial_states = last_decoder_states
        else:
        ...
        last_output, outputs, last_decoder_states = K.rnn(self.step, preprocessed_input,  initial_states,go_backwards=self.decoder.lstm.go_backwards,constants=constants, input_length=input_shape[1])
