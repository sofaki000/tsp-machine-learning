...
def evaluate(max_length,  batch_size,   lstm_width, num_blend_units,     num_training_loops,   loss_interval, optimizer):
    ...
    with tf.device("/cpu:0"):
        ...
        actual_index_dists = tf.placeholder(tf.float32,    name="ptr-out", shape=(num_indices, batch_size, input_length)) # I x B x J
        ...
        cell_dec = tf.contrib.rnn.LSTMCell(lstm_width,  use_peepholes=False, initializer=init)
...
for reset_loop_index in range(max_reset_retries):
    ...
    lstm_blend = 6
    result = evaluate(max_length=60, batch_size=1024,   lstm_width=lstm_blend,  num_blend_units=lstm_blend, num_training_loops=4000, loss_interval=50, optimizer=adagrad_optimizer)
